...
class Receive_vid(threading.Thread):
    ...
    def __init__(self, port, temp_path, name, hashfile, fold_name):
    ...
    def run(self):
        ...
        while True:
            ...
            if c_hash == 'end-of-upload-d-o-n-e-now-what-?':
                ...
                if not os.path.exists(PATH+self.fold_name):
                    os.makedirs(PATH+self.fold_name)

                vid = video_manager.Video_manager(path=self.temp_path, split_path=PATH+self.fold_name+'\\')
                ...
                mutex.acquire()
                threads[self.port] = (self.name, PATH+self.fold_name+'\\', vid.get_length(), self.hashfile, vid.get_num_of_parts(), vid.get_size())
                mutex.release()

                #print 'soooooooooooooooooooooooooooooooooo'
                os.remove(self.temp_path)

                break

            elif c_hash == 'canceled':
    ...
    def handle_request(self, dat, client):
        ...
        if dat == "" or dat == "exit":
            ...
            for search_dat in results:
                ...
                data_to_send += str(search_dat[0]) + ':!:' + str(search_dat[1]) + ':!:' + str(search_dat[2]) + ':<!>:'
            #self.send_to_customer('end results:', client)
            #removes an extra seperation sequance
            if not data_to_send == 'results:':
                data_to_send = data_to_send[:-5]
            self.send_to_customer(data_to_send, client)


        elif dat[:6] == 'Watch:':
            #in this case the client want to watch a video
            name = dat[6:dat.index(':!:')]
            part = dat[dat.index(':!:') + 3:]
            ...
            if sql.exists_in_base('MOVIE_NAME', name):
                ...
                stream = Streamer(0, port, fold, part, parts)
                stream.start()
            ...
            if port.isdigit() and int(port) in threads:
                threads[int(port)] = (1, part)
            mutex.release()
            ...
            if not sql.exists_in_base('HASH', f_hash) and not sql.exists_in_base('MOVIE_NAME', name) and len(name) > 0:
                port = rand_port()
                ...
                receiver = Receive_vid(port, TMP_UPLOAD + 'tmp_' + spaceless_name + str(port) + '.mp4', name, f_hash, spaceless_name)
                receiver.start()

                self.send_to_customer('upload_approved:' + str(port), client)
            elif not sql.exists_in_base('HASH', f_hash):
                self.send_to_customer('invalid:hash', client)
            else:
                self.send_to_customer('invalid:name', client)
    ...
    for current_socket in rlist:
        ...
        if current_socket is server_socket:
            ...
            try:
                ...
                com.handle_request('exit', current_socket)
            else:
